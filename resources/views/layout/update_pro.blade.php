    ...
    <section class="lg:w-2/3">
        <div class="bg-white rounded-xl shadow-md p-4 mb-4">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                <h2 class="text-2xl font-bold text-gray-900">{{ __('message.product') }}</h2>
        ...
        <div class="bg-white rounded-xl shadow-md p-4 sticky top-4">
            <div class="d-flex align-items-center mb-2 justify-content-between">
                <form action="{{ route('product.show') }}" method="get">
                    @csrf
                    <button class="btn px-1 py-0 fw-bolder d-flex align-items-center"><i
                            class="bi bi-caret-left-fill text-primary fs-2"></i>Back</button>
                </form>
                <h1 class="text-xl font-semibold text-blue-500 ml-2">Update</h1>
            </div>
            <form action="{{ route('product.update', $update->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <!-- Image -->
                <div class="mb-3 flex justify-center">
                    @if ($update->image)
                        <div class="mt-2">
                            <img id="imagePreview" src="{{ asset($update->image) }}" alt="Current Image"
                                class="w-68 h-48 object-cover rounded-lg border-2 border-gray-200 shadow-sm">
                        </div>
                    @endif
                </div>
                ...
                <div class="mb-3">
                    <label for="productName"
                        class="form-label fw-bold text-primary">{{ __('message.name') }}{{ __('message.product') }}</label>
                    <input type="text" value="{{ old('productName', $update->name) }}"
                ...
                <!-- Price -->
                <div class="mb-3">
                    <label for="productPrice" class="form-label fw-bold text-primary">{{ __('message.price') }} ($)</label>
                    <input type="number" value="{{ old('productPrice', $update->price) }}"
                        class="w-full px-4 py-2 border border-blue-300 rounded-lg  focus:ring-blue-500 focus:border-blue-500 transition duration-200" name="productPrice"
                        placeholder="{{ __('message.enter') }} {{ __('message.price') }}" required min="0"
                        step="0.01">
                </div>

                <!-- Quantity -->
                <div class="mb-3">
                    <label for="productQty" class="form-label fw-bold text-primary">{{ __('message.quantity') }}</label>
                    <input type="number" value="{{ old('productQty', $update->qty) }}"
                        class="w-full px-4 py-2 border border-blue-300 rounded-lg  focus:ring-blue-500 focus:border-blue-500 transition duration-200" name="productQty"
                        placeholder="{{ __('message.enter') }} {{ __('message.quantity') }}" required min="1">
                </div>

                <!-- Category -->
                <div class="mb-3">
                    <label class="form-label fw-bold text-primary">{{ __('message.category') }}</label>
                    <select name="productCategory" class="w-full px-4 py-2 border border-blue-300 rounded-lg  focus:ring-blue-500 focus:border-blue-500 transition duration-200" required>
                        <option disabled>{{ __('message.select') }} {{ __('message.category') }}</option>
                        <option value="phone" {{ $update->category === 'phone' ? 'selected' : '' }}>Phone</option>
                        <option value="laptop" {{ $update->category === 'laptop' ? 'selected' : '' }}>Laptop</option>
                        <option value="tablet" {{ $update->category === 'tablet' ? 'selected' : '' }}>Tablet</option>
                        <option value="watch" {{ $update->category === 'watch' ? 'selected' : '' }}>Watch</option>
                        <option value="headphones" {{ $update->category === 'headphones' ? 'selected' : '' }}>Headphones
                        </option>
                    </select>
                </div>
                ...
                <div class="mb-6">
                    <label for="productImage" class="block text-sm font-semibold text-gray-700 mb-2">
                ...
                <!-- Submit Button -->
                <button type="submit"
                    class="w-full bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-blue-700 focus:outline-none  focus:ring-blue-500 focus:ring-offset-2 transition duration-200">Update
                    Product</button>
            </form>
        </div>
        <script>
